...
-->
- Frontend: We chose to use React.js as our frontend, as it is the most popular front-end framework and has many plug-ins. It is also simple to get hands-on for basic usage. And the most famous free https cred service certbot has the best compatibility on Ubuntu.
...
- Server OS: We chose to use Ubuntu Linux as our server OS, not only because it is the most popular OS used in the industry, but it is also easy to manage with a good eco-system. 
